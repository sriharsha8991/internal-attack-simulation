...
description: Demonstrates and documents the engagement objectives — Domain Admin / Tier-0 compromise, crown-jewel access, MITRE ATT&CK coverage, and where in scope, ransomware / BEC / data-exfil simulations on isolated assets. Every action is evidence-producing, never destructive without per-action human ACK, and always reversible or simulation-only. Use when the krbtgt hash is in vault from DCSync, when a controlled principal is a confirmed Domain Admin on a DC, when crown-jewel access has been reached via the lateral chain, when the engagement requires final reporting artefacts (ATT&CK Navigator layer, kill-chain SVG, detection-gap analysis), or when scope authorises ransomware/BEC/exfil simulations on tagged isolated hosts. Covers full-domain DCSync proof extraction, DA shell capture on a DC, crown-jewel access listing (metadata only — no real exfil), MITRE ATT&CK mapping, isolated-host ransomware simulation, BEC mailbox simulation, business-process disruption demo, dummy data exfil, hybrid cloud impact (on-prem DA → cloud GA), and purple-team replay.
stage: impact
agent: ImpactAgent

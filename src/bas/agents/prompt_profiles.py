...
_BASE_EVALUATOR = (
    ...
    "\n"
    "SKILL ADHERENCE\n"
    "  The skill playbook is the MANDATORY baseline. The plan must COVER every\n"
    "  step the skill prescribes (each maps to at least one ability/stage). A\n"
    "  dropped skill step is a DEFECT unless a named memory fact already\n"
    "  satisfies it and the rationale says so. Extra abilities BEYOND the skill\n"
    "  are WELCOME — do not penalise useful additions that advance the objective\n"
    "  and stay within the tool_allowlist. The planner may reorder/merge/improve\n"
    "  step implementations; judge by 'is every prescribed step covered?', not\n"
    "  by verbatim command match.\n"
    "\n"
    "WHAT TO CHECK\n"
    "  * Skill step coverage: every prescribed skill step maps to an ability, OR\n"
    "    is justified as already-satisfied by a named memory fact. Flag any\n"
    "    dropped step as a mismatch.\n"
    "  * Phase objective met (the skill's GOAL).\n"
...
_DISCOVERY_NOTES = (
    ...
    "  * IMPORTANT: Cover every numbered skill step. You may MERGE or REORDER\n"
    "    them for efficiency, but only SKIP a step that memory already\n"
    "    satisfies (cite the fact). Adding extra discovery that sharpens the\n"
    "    picture is encouraged.\n"
